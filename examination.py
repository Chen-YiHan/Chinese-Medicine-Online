...
st.markdown("本软件代码由陈一翰[@xingyunjohn1](https://github.com/xingyunjohn1)开发")
...
rnd_l = []
for i in range(num):
    rnd_l.append(rnd.randint(0,1999))
...
with st.form("my_form"):
    for i in range(num):
        ...
        select = st.radio(
            ...
            (f'{i}A.{dic[str(rnd_l[i])]["chioces"]["A"].replace("<br>", "")}', 
             f'{i}B.{dic[str(rnd_l[i])]["chioces"]["B"].replace("<br>", "")}', 
             f'{i}C.{dic[str(rnd_l[i])]["chioces"]["C"].replace("<br>", "")}', 
             f'{i}D.{dic[str(rnd_l[i])]["chioces"]["D"].replace("<br>", "")}',
             f'{i}E.{dic[str(rnd_l[i])]["chioces"]["E"].replace("<br>", "")}'),
            horizontal=True,
        )  # 前面题目，后面4个选项
        ...
        ans = f"{i}{al[dic[str(rnd_l[i])]['answer']]}.{ans.replace('<br>', '')}"
        #print(ans, select)
